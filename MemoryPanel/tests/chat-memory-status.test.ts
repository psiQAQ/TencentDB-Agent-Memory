...
function createApp(opts: { manual?: boolean; failedPath?: string } = {}) {
  ...
  const postEnvelope = vi.fn(async (path: string) => {
    ...
    if (path === '/v3/conversation/query') return { code: 0, data: { messages: [], total: 4 } };
    if (path === '/v3/atomic/query') return { code: 0, data: { items: [], total: 3 } };
    if (path === '/v3/scenario/ls') return { code: 0, data: { entries: [], total: 2 } };
    return { code: 0, data: { content: '# persona' } };
  });
  const deps = {
...
describe('chat-memory/status', () => {
  it('returns counts without memory content', async () => {
    ...
    expect(JSON.stringify(envelope)).not.toContain('persona');
  });

  it('marks a failed layer as partial instead of zero', async () => {
    const { app, blockId } = createApp({ failedPath: '/v3/atomic/query' });
