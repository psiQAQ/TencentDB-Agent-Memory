...
export function TeamAtlasPage() {
  ...
  const projection = useMemo(
    () =>
      ir
        ...
        : null,
    [ir, selectedTeamIds, query, assetType, showUnboundAssets, showOtherOwners],
  );
  ...
  const directNodeIds = useMemo(
    () => directVisualNodeIds(displayedNodes, interactionEdges, activeNode),
    [activeNode, displayedNodes, interactionEdges],
  );
  ...
  const nodeCaption = useCallback(
    (node: TeamAtlasNode) => {
      ...
      if (node.type === 'task')
        return `Agent ${outgoing.filter((edge) => edge.type === 'assigned_to').length} · ${t('atlas.creator')} ${shortId(node.metadata?.creator_user_id)}`;
  ...
  const warningByNode = new Map(
    ir.warnings.filter((warning) => warning.node_id).map((warning) => [warning.node_id!, warning]),
  );
  const partialSources = ir.warnings.filter((warning) => warning.code === 'SOURCE_PARTIAL').length;
  const teamOptions = ir.nodes
    .filter((node) => node.type === 'team')
    .sort((a, b) => a.label.localeCompare(b.label));
  const selectedTeamLabel =
    selectedTeamIds.length === 1
      ? (teamOptions.find((team) => team.entity_id === selectedTeamIds[0])?.label ??
        t('atlas.selectTeams'))
              ...
              >
                {teamOptions.map((team) => (
                  <label key={team.id}>
                    <input
                      type="checkbox"
                      checked={selectedTeamIds.includes(team.entity_id)}
                      onChange={() => toggleTeam(team.entity_id)}
                    />
                    {team.label}
                  </label>
                ))}
              ...
              <g className="team-atlas-edges">
                {interactionEdges.map((edge) => {
                  ...
                  const active = Boolean(
                    logicalActiveId &&
                    (edge.source === logicalActiveId || edge.target === logicalActiveId),
                  );
                  return (
                    <g
                      key={edge.id}
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        {node.type === 'task' ? (
                          <>
                            <text className="team-atlas-node-type" x="12" y="14">
                              {t('atlas.type.task')}
        ...
        <aside className="team-atlas-passport" aria-live="polite">
          ...
          {activeNode ? (
            <>
              ...
              {logicalActiveId && warningByNode.get(logicalActiveId) && (
                <p className="team-atlas-passport-warning">
                  {warningByNode.get(logicalActiveId)?.message}
                </p>
              )}
              <button
                type="button"
