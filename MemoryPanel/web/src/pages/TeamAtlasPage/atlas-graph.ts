...
export function directVisualNodeIds(
  ...
  for (const edge of edges) {
    ...
    if (edge.type === 'assigned_to') {
      if (activeNode.type === 'task' && activeNode.parent_agent_id === edge.target)
        ids.add(edge.target);
      if (activeNode.type === 'agent' && edge.target === activeLogicalId) {
        for (const node of nodes) {
          if (
            node.type === 'task' &&
            node.logical_id === edge.source &&
            node.parent_agent_id === activeLogicalId
          )
            ids.add(node.id);
        }
      }
...
export function projectAtlas(
  ...
  if (options.showUnboundAssets === false) {
    ...
    const ownBoundAssetIds = new Set(
      ir.edges
        .filter(
          (edge) =>
            (edge.type === 'owns' || edge.type === 'fixed_binding') && ownAgentIds.has(edge.source),
        )
  ...
  if (options.focusAgentId) {
    ...
    if (agent?.team_id) {
      ...
      for (const member of ir.nodes.filter(
        (node) => node.type === 'identity' && node.team_id === agent.team_id,
      ))
        ids.add(member.id);
    }
    ...
    if (nodes.length > maxNodes) {
      ...
      nodes = stableNodes([
        ...structural.slice(0, Math.max(1, maxNodes - groups.length)),
        ...groups,
      ]);
    }
  ...
  if (options.assetType && options.assetType !== 'all') {
    const matched = new Set(
      nodes
        .filter((node) => node.type === options.assetType || !ASSET_TYPES.has(node.type))
        .map((node) => node.id),
    );
...
export function summarizeAtlas(ir: TeamAtlasIR, teamIds?: string[]): AtlasSummaryCard[] {
  ...
  const visibleEdges = ir.edges.filter(
  ...
  for (const edge of visibleEdges) {
    ...
    if ((edge.type === 'owns' || edge.type === 'fixed_binding') && ownAgentIds.has(edge.source))
      mine.add(edge.target);
  }
  ...
  for (const team of teams) {
    ...
    const memberSections = members.map((member) => {
      const ownedAgents = agents.filter(
        (agent) => agent.metadata?.owner_user_id === member.entity_id,
      );
      const agentGroups = ownedAgents.map((agent) => {
        ...
        return {
          agent,
          tasks: assignedTasks,
          height: nodeHeight + assignedTasks.length * (taskHeight + taskGap),
        };
      });
      const looseTasks = tasks.filter(
        (task) =>
          !assignedTaskIds.has(task.id) && task.metadata?.creator_user_id === member.entity_id,
      );
      const agentHeight =
        agentGroups.reduce((sum, group) => sum + group.height, 0) +
        Math.max(0, agentGroups.length - 1) * agentGap;
      const looseHeight = looseTasks.length * (taskHeight + taskGap);
      return {
        member,
        agentGroups,
        looseTasks,
        height: Math.max(nodeHeight, agentHeight + looseHeight),
      };
    });
    const memberHeight =
      memberSections.reduce((sum, section) => sum + section.height, 0) +
      Math.max(0, memberSections.length - 1) * memberGap;
    ...
    for (const section of memberSections) {
      memberCardYs.push(
        section.agentGroups.length > 0
          ? memberCursor
          : memberCursor + (section.height - nodeHeight) / 2,
      );
      memberCursor += section.height + memberGap;
    }
    const teamCenterY =
      memberCardYs.length > 0
    ...
    for (const [sectionIndex, section] of memberSections.entries()) {
      ...
      for (const group of section.agentGroups) {
        ...
        for (const task of group.tasks) {
          positioned.push({
            ...task,
            id: `${task.id}@${group.agent.id}`,
            logical_id: task.id,
            parent_agent_id: group.agent.id,
            x: TYPE_X.task,
            y: taskY,
            width: nodeWidth - 12,
            height: taskHeight,
    ...
    assets.forEach((asset, index) => {
      ...
      positioned.push({
        ...asset,
        x: TYPE_X[asset.type],
        y: assetStart + index * (nodeHeight + assetGap),
        width: nodeWidth,
        height: nodeHeight,
...
export function edgeGeometry(
  ...
  const mid =
    corridorStart + ((index + 1) / (peerEdges.length + 1)) * (corridorEnd - corridorStart);
  return {
