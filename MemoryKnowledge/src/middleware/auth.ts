 ...
 *   - KNOWLEDGE_SERVICE_KEY 为空  → 一般端点不启用鉴权（本地开发）；生命周期端点仍鉴权；
 *   - KNOWLEDGE_SERVICE_KEY 非空  → /v3 下除只读白名单和生命周期端点外均要求
 ...
 * 生命周期端点使用独立的 KNOWLEDGE_LIFECYCLE_AUTH_TOKEN，由路由层校验。
...
/** These routes validate a separate lifecycle credential in their own handlers. */
export function isLifecyclePath(method: string, path: string): boolean {
  return method === "POST" && (LIFECYCLE_POST_PATHS.has(path) || path.startsWith("/internal/lifecycle/"));
}
...
export function createServiceAuthMiddleware(
  ...
  return async (c, next) => {
    ...
    if (isLifecyclePath(c.req.method, path)) return next();
