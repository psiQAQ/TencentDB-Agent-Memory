...
- **Observability & usage reporting**: three independent channels — Opik, Langfuse, and ClickHouse. Langfuse uses one random trace per proxy request without stable cross-request identity; ClickHouse writes fixed-redaction identity columns plus aggregate/categorical metrics; Opik follows its own request/fork trace contract. Any one channel failing does not affect the business path.
