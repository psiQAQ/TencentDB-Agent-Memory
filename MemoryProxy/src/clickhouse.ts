...
export function buildClickHouseRow(entry: ClickHouseWriteEntry): ClickHouseRow | null {
  ...
  return {
    ...
    user_id: entry.keyId ?? "",
    upstream_url: privacySafeOrigin(entry.upstreamUrl),
...
export function buildSessionInitLogRow(input: SessionInitLogInput): SessionInitLogRow {
  ...
  return {
    ...
    session_key: privacySafeSessionId(input.sessionKey),
    space_id: input.spaceId ?? "",
    user_id: input.userId ?? "",
    team_id: privacySafeText(input.teamId),
    agent_id: privacySafeText(input.agentId),
