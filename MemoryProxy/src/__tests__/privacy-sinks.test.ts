...
describe("privacy-safe observability sinks", () => {
  ...
  it("keeps typed usage for ClickHouse classification and cost before final row redaction", async () => {
    ...
    expect(row?.routed_from).toBe("original-model");
    expect(rawInsert).toBeUndefined();
    expect(row?.session_key).toBe("[redacted]");
    expect(row?.user_input).toBe("[redacted]");
    expect(row?.upstream_request_id).toBe("");
    expect(row?.upstream_url).toBe("https://tokenhub.invalid");
    expect(row?.model_id).toBe("priced-model");
    expect(row?.user_id).toBe(PRIVATE_VALUE);
    expect(row?.space_id).toBe(PRIVATE_VALUE);
  ...
  it("retains business dimensions but stores no session, prompt, path, or request id", () => {
    const entry = {
      timestamp: new Date().toISOString(),
      event: "usage",
      modelId: "business-model",
      keyId: "business-user",
      sessionKey: PRIVATE_VALUE,
      userInput: PRIVATE_VALUE,
      upstreamUrl: `https://upstream.invalid/${PRIVATE_VALUE}`,
      ...
      routedFrom: "business-routed-model",
    ...
    expect(rows.every((row) => row !== null && row.model_id === "business-model")).toBe(true);
    expect(rows.every((row) => row !== null && row.user_id === "business-user")).toBe(true);
    expect(rows.every((row) => row !== null && row.space_id === "business-space")).toBe(true);
    expect(rows.every((row) => row !== null && row.routed_from === "business-routed-model")).toBe(true);
    expect(rows.every((row) => row !== null && row.session_key === "[redacted]")).toBe(true);
    expect(rows.every((row) => row !== null && row.user_input === "[redacted]")).toBe(true);
    expect(rows.every((row) => row !== null && row.upstream_url === "https://upstream.invalid")).toBe(true);
    expect(rows.every((row) => row !== null && row.upstream_request_id === "")).toBe(true);
  });
  ...
  it("runtime-validates opaque extension log control fields", () => {
    ...
    expect(normalizedTimestamp?.stream).toBe(false);
  });
