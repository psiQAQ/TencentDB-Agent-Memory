...
describe("session cache identity isolation", () => {
  ...
  it.each(["l2a", "l2b"] as const)(
    ...
    async (failingLayer) => {
      ...
      await expect(store.getOrRecover(key, identity, {
        metadataClient: { getAgent, getTask } as never,
      })).rejects.toMatchObject({ name: "SessionContextUnavailableError" });
      expect(store.get(key)).toBeUndefined();
  ...
  it("restores a failed-delete tombstone when its replacement set also fails", async () => {
    ...
    })).rejects.toMatchObject({ name: "SessionStoreUnavailableError" });
  ...
  it("preserves a concurrent persisted raw owner when another claim rolls back", async () => {
    ...
    releaseWrite();
    await expect(first).rejects.toMatchObject({ name: "SessionStoreUnavailableError" });
  ...
  it.each(["l2a", "l2b"] as const)(
    ...
    async (failingLayer) => {
      ...
      store.bind(key, fullIdentity);
      releaseWrite();
      await expect(write).rejects.toMatchObject({ name: "SessionStoreUnavailableError" });
  ...
  it.each([
    ...
    ["skill", "/skill-bridge/v3/skill/list", 403],
  ...
  it.each([
    ...
    ["memory", "claude-code", "/memory-bridge/v3/scenario/read"],
    ["memory", "dsh", "/memory-bridge/v3/scenario/read"],
    ["memory", "opencode", "/memory-bridge/v3/scenario/read"],
    ["memory", "pi", "/memory-bridge/v3/scenario/read"],
    ["memory", "codex", "/memory-bridge/v3/scenario/read"],
    ["memory", "workbuddy", "/memory-bridge/v3/scenario/read"],
    ["skill", "codebuddy", "/skill-bridge/v3/skill/list"],
    ["skill", "openai", "/skill-bridge/v3/skill/list"],
    ["skill", "claude-code", "/skill-bridge/v3/skill/list"],
    ["skill", "dsh", "/skill-bridge/v3/skill/list"],
    ["skill", "opencode", "/skill-bridge/v3/skill/list"],
    ["skill", "pi", "/skill-bridge/v3/skill/list"],
    ["skill", "codex", "/skill-bridge/v3/skill/list"],
    ["skill", "workbuddy", "/skill-bridge/v3/skill/list"],
  ] as const)(
