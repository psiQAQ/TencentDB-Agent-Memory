...
async function handleConversationAdd(body: unknown, auth: V2AuthContext, requestId: string, deps: V2RouterDeps): Promise<ApiResponseEnvelope> {
  ...
  if (store.insertL0Batch && !embedding) {
    const persistedCount = await store.insertL0Batch(acceptedRecords);
    if (persistedCount !== acceptedRecords.length) {
      deps.logger.error(`${TAG} L0 batch persistence incomplete`);
    ...
    for (const record of acceptedRecords) {
      ...
      if (!persisted) {
        deps.logger.error(`${TAG} L0 persistence failed for ${record.id}`);
        return errorEnvelope(503, "Failed to persist conversation message", requestId);
      }
    }
  }
