...
export class MongoSkillStore implements ISkillStore {
  ...
  async transferOwnerAgent(
    ...
    try {
      // Ownership spans every version, so a failed write must roll back all rows.
      return await session.withTransaction(async () => {
        const scope = { skill_id: skillId, team_id: teamId };
        const head = await coll.findOne({ ...scope, is_head: true }, { session, sort: { version: -1 } });
        if (!head) throw new SkillStoreError("SKILL_NOT_FOUND");
        if (head.owner_agent_id === toAgentId) return 0;
        if (head.owner_agent_id !== fromAgentId) throw new SkillStoreError("SKILL_NOT_OWNER");
        const changedOwner = await coll.findOne({ ...scope, owner_agent_id: { $ne: fromAgentId } }, { session });
        if (changedOwner) throw new SkillStoreError("SKILL_NOT_OWNER");
        const result = await coll.updateMany(
          { ...scope, owner_agent_id: fromAgentId },
          { $set: { owner_agent_id: toAgentId, updated_at_ms: this.now() } },
          { session },
        );
