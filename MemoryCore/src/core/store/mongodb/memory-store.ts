...
import type {
  ...
  MemoryOwnerTransferInput,
  MemoryOwnerTransferResult,
  MemoryIntegrityScope,
} from "../types.js";
...
export class MongoMemoryStore implements IMemoryStore {
  ...
  async upsertL1(record: MemoryRecord, _embedding?: Float32Array): Promise<boolean> {
    ...
    // Reprocessing historical records must not undo an ownership transfer.
    const { _id, owner_user_id, ...fields } = doc;
    await coll.updateOne(
      { _id, team_id: doc.team_id, agent_id: doc.agent_id } as never,
      { $set: fields, $setOnInsert: { owner_user_id } } as never,
      { upsert: true },
    );
    return true;
  }
  ...
  private l1CountQuery(filter?: L1CountFilter): Record<string, unknown> {
    ...
    if (filter.userId !== undefined) q.owner_user_id = filter.userId;
  ...
  private async runSearch(
    ...
    const pipeline: Record<string, unknown>[] = [
      ...
      // Search indexes are eventually consistent; enforce current ownership
      // against canonical documents before limiting the visible result page.
      { $match: isolationToMatch(filter) },
  ...
  async transferMemoryOwner(input: MemoryOwnerTransferInput): Promise<MemoryOwnerTransferResult> {
    ...
    try {
      return await session.withTransaction(async () => {
        const match = { team_id: teamId, agent_id: agentId, owner_user_id: fromOwner };
        const update = { $set: { owner_user_id: toOwner } };
        const l0Result = await l0.updateMany(match, update, { session });
        const l1Result = await l1.updateMany(match, update, { session });
        return { l0Updated: l0Result.modifiedCount, l1Updated: l1Result.modifiedCount };
      });
    } finally {
  ...
  async listMemoryIntegrityScopes(): Promise<MemoryIntegrityScope[]> {
    const grouped = new Map<string, MemoryIntegrityScope>();
    for (const [name, countKey] of [[COLLECTIONS.L0, "l0Count"], [COLLECTIONS.L1, "l1Count"]] as const) {
      const coll = await this.coll(name);
      const rows = await coll.aggregate([{
        $group: {
          _id: { team_id: "$team_id", agent_id: "$agent_id" },
          owners: { $addToSet: { $ifNull: ["$owner_user_id", "$user_id"] } },
          count: { $sum: 1 },
        },
      }]).toArray();
      for (const row of rows) {
        const teamId = String(row._id?.team_id ?? "");
        const agentId = String(row._id?.agent_id ?? "");
        const key = `${teamId}\0${agentId}`;
        const scope = grouped.get(key) ?? { teamId, agentId, ownerUserIds: [], l0Count: 0, l1Count: 0 };
        scope.ownerUserIds = [...new Set([...scope.ownerUserIds, ...row.owners.filter((owner: unknown) => typeof owner === "string" && owner)])];
        scope[countKey] += Number(row.count ?? 0);
  ...
  async clearMemoryContent(filter: MemoryContentClearFilter): Promise<MemoryContentClearResult> {
    ...
    if (filter.userId !== undefined) {
      match.owner_user_id = filter.userId;
      profileMatch.user_id = filter.userId;
    }
