...
require_vars \
  ...
  MEMORY_LLM_BASE_URL MEMORY_LLM_API_KEY MEMORY_LLM_MODEL \
  PROXY_UPSTREAM_MODEL KNOWLEDGE_PUBLIC_BASE_URL MEMORY_CORE_GATEWAY_API_KEY
...
[[ "$KNOWLEDGE_SERVICE_KEY" != "$MEMORY_CORE_GATEWAY_API_KEY" ]] \
  || die "KNOWLEDGE_SERVICE_KEY 必须与生命周期凭据不同"
...
$DOCKER run -d --name "$CONTAINER" \
  ...
  -p "${PANEL_BIND_HOST:-0.0.0.0}:${PANEL_PORT}:8125" \
  -p "${KNOWLEDGE_BIND_HOST:-0.0.0.0}:${KNOWLEDGE_PORT}:8424" \
  ...
  -e KNOWLEDGE_LIFECYCLE_AUTH_TOKEN="$MEMORY_CORE_GATEWAY_API_KEY" \
